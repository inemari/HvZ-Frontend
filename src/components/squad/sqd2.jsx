...
const SquadInformation = ({ squadId, locationHubConnection }) => {
  const [newMessage, setNewMessage] = useState("hei");
  ...
  useEffect(() => {
    if (locationHubConnection) {
      locationHubConnection.on("ReceiveMessage", (playerId, message) => {
        setReceivedMessages([...receivedMessages, `${playerId}: ${message}`]);
      });
    }
  }, [locationHubConnection, receivedMessages]);
  ...
  const sendMessage = () => {
    ...
    if (
      ...
      console.log("trueSend")
      locationHubConnection.invoke("SendMessage", newMessage).catch((error) => {
  ...
  const handleLeaveMarker = async (event) => {
    ...
    sendMessage();
    console.log("try to send message from handleLeaveMarker");


      console.log("Trying to connect to LocationHub...");
      // Notify others in the squad about the marker location updat
      ...
      if (
        ...
        locationHubConnection.state === signalR.HubConnectionState.Connected
          ...
          console.log(playerId, xCoordinate, yCoordinate)
        locationHubConnection
          .invoke(
            "SendLocationUpdate",
            parseInt(playerId),
